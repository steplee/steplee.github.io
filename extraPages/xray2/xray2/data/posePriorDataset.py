...
#
# doubleDataset: the dataset size is actually 2x the original size. Any indices > n0 will have x axis flipped.
# WARNING: But isn't that incorrect??? The fact that the left leg is on the left _is_ meaningful to conditional data...
#
class PosePriorDataset(Dataset):
    def __init__(self, file, masterScale=1, doubleDataset=False):
        ...
        print(f' - PosePriorDataset(d={self.data.shape}, inds={self.inds.shape}, len={len(self)})')
