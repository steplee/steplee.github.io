...
class SurfaceRenderer(SingletonApp):
    def __init__(self, wh, name='MagMap'):
        ...
        self.eye = np.array((0,0,-4),dtype=np.float32)

        self.R = np.diag(np.array((1,-1,-1),dtype=np.float32))
    ...
    def render(self):
        ...
        glLoadIdentity()
        glFrustum(-u*n,u*n,-v*n,v*n,n,1600)
