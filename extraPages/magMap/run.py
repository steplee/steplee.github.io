...
# def get_frames(file, stride=10,N=99):
def get_frames(file, stride=10,N=4):
    good = True
    frames = []
...
def decomposeEssential(E,pts1,pts2):
    ...
    if R2[2,2] > R1[2,2]:
        T = np.concatenate((R2, t), 1)
    else:
        T = np.concatenate((R1, t), 1)
    # T = np.concatenate((R1.T, -R1@t), 1)
...
def triangulateAbsolute(A,B,K,pts1,pts2):
    ...
    if 1:
        ...
        pts4 = cv2.triangulatePoints(K@A[:3],K@B[:3],pts1.T,pts2.T).T
        pts = pts4[:,:3] / pts4[:,3:]
...
class Runner:
    ...
    def construct_graph(self, linearFrameGraph, first_H_info):
        ...
        NT = NC*camStateSize + 3*NK # Total tangent size
        ...
        for entry in linearFrameGraph:
            ...
            tstPts = tstPts[:,:2] / tstPts[:,2:]
            print('Projected error (px) of new pose with pnp points:\n',tstPts - pnpPtsObsb)
        ...
        for i in range(32):
            ...
            coo = torch.stack((torch.LongTensor(a),torch.LongTensor(b)),0)
            val = torch.FloatTensor(c)
            rmse = (torch.stack(newRes,0).view(-1)**2).sum().sqrt()
            print(' - RMSE', rmse)

            M = len(newAlpha)
            Alpha = torch.sparse_coo_tensor(torch.arange(M).view(1,-1).repeat(2,1), torch.FloatTensor(newAlpha))
            J = torch.sparse_coo_tensor(coo,val).coalesce()
            print(J, torch.stack(newRes,0).view(-1).shape)

            Jtres = J.mT @ Alpha @ torch.stack(newRes,0).view(-1)
            # Jtres = torch.stack(newRes,0).view(-1)
            print(' - Jtres norm', Jtres.norm())

            dx0 = torch.zeros(J.shape[1])
            print('J', J.shape)
            print('Jtres', Jtres.shape)
            print('dx0', dx0.shape)
            print('x', x.shape)
            #dx = solve_cg_AtA(J, Jtres, dx0, preconditioned=True, debug=True, T=100) # NOT WORKING.
            A = J.mT@Alpha@J
